...
function FacultyRegistration() {
    ...
    let {facultyRegis} = useContext(ServerContext);
    let [facultyId, setFacultyId] = useState("");
    let [password, setPassword] = useState("");

    let handleReg = async(e)=>
        {
            e.preventDefault();
            
            try {
                let res = await facultyRegis(facultyId, password);
                console.log(res);
                // if(res.status == 200)
                // {
                //     alert("Login Success");
  ...
  return (
    <div className='m-5'>
      <form className='d-flex justify-content-center align-items-center' onSubmit={handleReg}>
